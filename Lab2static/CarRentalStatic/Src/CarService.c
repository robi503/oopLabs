...
CarService createService(MyList* l) {
	CarService cs;
	cs.l = l;
	return cs;
}
...
MyList* filterCarsByCategory(CarService* cs, const char* category) {
	MyList filteredList = createEmpty(); // Create an empty list to store filtered cars
...
MyList* sortCars(CarService* cs, int(*cmpFct)(Car, Car), int direction) {
	MyList sortedList = createEmpty(); // Create an empty list to store sorted cars
