...
Car createCar(const char* licensePlate, const char* model, const char* category) {
    Car car;
    strcpy_s(car.licensePlate, sizeof(car.licensePlate), licensePlate);
    for (int i = 0; i < strlen(car.licensePlate); i++) {
        car.licensePlate[i] = toupper(car.licensePlate[i]);
    }
    strcpy_s(car.model, sizeof(car.model), model);
    strcpy_s(car.category, sizeof(car.category), category);
    car.rented = 0;
    return car;
}
